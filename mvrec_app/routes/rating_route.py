...
from mvrec_app.model.queury_db import store_render_movies, get_render_movies, reset_client_db, store_naver_api_movies
from mvrec_app.utils.main_funcs import cosine_sim
...
def add_movie_rating():
    ...
    return redirect(url_for('main.mv_recommend', _anchor=scroll_idx, movie_list=str(movie_list)))
...
def init_first_movies():
    ...
    return redirect(url_for('main.mv_recommend', movie_list=str(movie_list_api)))
...
def init_rating_table():
    ...
    movie_list = []
    for movie in RatingClient.query.all():
        movie_list.append({"title_kr":movie.naver_api_info.title_kr, 
                                "id":movie.naver_api_info.movie_id,
                                "title_original":movie.naver_api_info.title_original,
                                "image":movie.naver_api_info.image,
    ...
    return redirect(url_for('main.client_rating', movie_list=str(movie_list)))
